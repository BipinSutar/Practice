...
public class DrawingCololePragram {
	static Canvas1 canvas;
	...
	private static void draw(String command) throws NumberFormatException, InterruptedException {
		...
		try {
			...
			case 'C':
				cmd = command.split(" ");
				canvas = new Canvas1(Integer.parseInt(cmd[1]), Integer.parseInt(cmd[2]));
				canvas.render();
				break;
			case 'L':
				cmd = command.split(" ");
			...
			case 'R':
				...
				canvas.drawRectangle(Integer.parseInt(cmd[1]), Integer.parseInt(cmd[2]), Integer.parseInt(cmd[3]),
						Integer.parseInt(cmd[4]), 'X');
				canvas.render();
				break;
			case 'B':
